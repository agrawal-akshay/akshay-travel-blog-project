...
export default function EditInsightPage() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/posts/${id}`)
      ...
      .catch(err => {
        console.error(err);
        alert('An error occurred loading post.');
      })
      .finally(() => setLoading(false));
  }, [id, router]);

  // Load content into contenteditable once data is fetched and loaded
  useEffect(() => {
    if (!loading && editorRef.current && formData.content) {
      const normalized = normalizeEditorHtml(formData.content);
  ...
  const handleSubmit = async (e: any) => {
    ...
    try {
      ...
      console.error(err);
      alert("An error occurred saving the post.");
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div className="max-w-7xl mx-auto p-6 md:p-8 min-h-screen">
      <div className="mb-8">
        <Link href="/admin/posts" className="inline-flex items-center text-sm font-semibold text-gray-500 hover:text-blue-600 transition-colors gap-1.5 mb-4">
          <ArrowLeft className="w-4 h-4" /> Back to insights
        </Link>
        <h2 className="text-3xl font-bold text-gray-900 font-serif">Edit Insight</h2>
      </div>
      ...
      <form onSubmit={handleSubmit} className="bg-white border border-gray-200 rounded-2xl p-8 shadow-sm">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
          ...
          <div>
            ...
            <select name="category" value={formData.category} onChange={handleChange} className="w-full border border-gray-200 rounded-lg px-4 py-2.5 text-sm focus:border-blue-500 focus:ring-2 focus:ring-blue-200 outline-none">
        ...
        <div className="pt-4 border-t border-gray-100">
           <button type="submit" disabled={saving} className="bg-blue-600 disabled:bg-blue-800 text-white px-10 py-3.5 rounded-lg font-bold hover:bg-blue-700 hover:shadow-lg transition-all w-full md:w-auto flex items-center justify-center gap-2 cursor-pointer">
             {saving ? (
               <>
