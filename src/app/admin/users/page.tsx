...
import { Users as UsersIcon, Search, Trash2, Calendar, Mail, UserPlus, X } from 'lucide-react';
  ...
  return (
    <div className="max-w-7xl mx-auto p-6 md:p-8 space-y-8 bg-zinc-50 min-h-screen font-sans">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h1 className="text-3xl font-bold text-gray-900 font-serif">Users Directory</h1>
          <p className="text-gray-500 text-sm mt-1">Manage registered readers and newsletter subscribers.</p>
        </div>
        <button 
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-white border border-gray-200 p-6 rounded-2xl shadow-sm flex items-center justify-between">
          <div>
            <span className="text-gray-400 font-semibold text-xs uppercase tracking-wider block">Total Users</span>
      ...
      <div className="bg-white border border-gray-200 rounded-2xl shadow-sm overflow-hidden">
        ...
        {loading ? (
                ...
                {filteredUsers.map((user) => (
                  <tr key={user._id} className="hover:bg-zinc-50/50 transition-colors">
                    ...
                    <td className="py-4 px-6">
                      <span className="bg-zinc-100 text-zinc-700 text-xs px-2.5 py-1 rounded-full font-semibold border border-zinc-200">
                        {user.role}
                      </span>
                    </td>
          ...
          <div className="py-20 text-center text-gray-400">
            ...
            <p>No user accounts matching search query.</p>
          </div>
        )}
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
          <div className="bg-white rounded-2xl max-w-md w-full border border-gray-200 overflow-hidden shadow-2xl animate-in fade-in zoom-in-95 duration-200">
            <div className="flex justify-between items-center px-6 py-4 border-b border-gray-200 bg-gray-50">
              <h3 className="font-bold text-gray-900 font-serif">Add New User</h3>
              <button 
